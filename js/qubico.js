$(document).ready(function () {
			...
			if ($("#sequence").length) {
				...
				sequence.afterLoaded = function () {
					$('#sequence').delay(1000).animate({
						'opacity': 1
					});

					setTimeout(function () {
						$("#sequence h2").fitText(0.9, {
							minFontSize: '24px',
							maxFontSize: '56px'
						});
						$("#sequence p").fitText(1.2, {
							minFontSize: '16px',
							maxFontSize: '28px'
						});
					}, 500);
			...
			$('a.scrollto').click(function (e) {
				...
				$('.text-slider').flexslider({
					...
					before: function (slider) {
						if ($('.jumbotron.bg-slider').length) $('.jumbotron.bg-slider').backstretch("show", slider.animatingTo);
					},
					start: function () {
				...
				if ($('.jumbotron.bg-slider').length) {
					...
					$('.jumbotron.bg-slider').backstretch(bgSlides, {
						fade: 500,
				...
				$('.jumbotron .next-slide').click(function () {
					if ($('.jumbotron .text-slider').length) {
						$('.jumbotron .text-slider').flexslider('next');
						$('.jumbotron .text-slider').flexslider('play');
					} else if ($('.jumbotron.bg-slider').length) $('.jumbotron.bg-slider').backstretch('next');
				})

				$('.jumbotron .prev-slide').click(function () {
					if ($('.jumbotron .text-slider').length) {
						$('.jumbotron .text-slider').flexslider('prev');
						$('.jumbotron .text-slider').flexslider('play');
				...
				Skills Charts
				==============================================*/
				var firstLoad = true; $('.skills').waypoint(function () {
					if (firstLoad) {
						$('.countTo').each(count);
						firstLoad = false;
					}
				...
				var moreSet = 1; $('#ajax-load').click(function (e) {
					e.preventDefault();
					...
					$.get(moreLink, function (data) {
						...
						$('.projects-container .loaded-item').removeClass('loaded-item')
							.find('.project-thumb .main-link,.project-thumb .link').click(function (e) {
								...
								if ($(this).parents('.project-item').is('.filtered')) {
									return false;
								}
